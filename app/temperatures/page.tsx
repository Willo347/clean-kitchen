...
function EquipmentCard({
  ...
  return (
    <div
      className={`
        rounded-[24px] border backdrop-blur-xl overflow-hidden transition-all duration-300
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <span className="text-[10px] font-bold uppercase tracking-widest text-white/25 mb-1 block">
              {equipment.zone}
            </span>
        ...
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/[0.05]">
          <div className="flex items-center gap-2">
            ...
            {!status && (
              <span className="text-white/25 text-xs font-bold flex items-center gap-1">
                <Clock size={10} /> AUCUN RELEVÉ
              </span>
            )}
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => onQuickAdd(equipment.name)}
...
export default function TemperaturesPage() {
  ...
  const fetchData = useCallback(async () => {
    const { data: equipmentsData, error: equipError } = await supabase
      .from("equipments").select("*").order("id");
    if (equipError) { addToast(`Erreur équipements : ${equipError.message}`, "error"); return; }

    const { data: logsData, error: logsError } = await supabase
      .from("temperature_logs").select("*").order("created_at", { ascending: false });
  ...
  async function addTemperature() {
    if (!selectedEquipment || !temperature) {
      addToast("Sélectionnez un équipement et entrez une température", "error"); return;
    }
    ...
    if (eq && (tempNum < eq.temp_min || tempNum > eq.temp_max)) {
      addToast(`⚠️ Alerte HACCP — ${selectedEquipment} hors zone (${tempNum}°C)`, "error");
    } else {
  ...
  async function submitQuickAdd() {
    ...
    if (eq && (tempNum < eq.temp_min || tempNum > eq.temp_max)) {
      addToast(`⚠️ Alerte HACCP — ${quickAddEquipment} hors zone (${tempNum}°C)`, "error");
    } else {
  ...
  async function exportPDF() {
    ...
    if (filteredLogs.length === 0) {
      addToast("Aucun relevé trouvé avec ces filtres", "error");
    ...
    doc.text("HACCP — Rapport de températures", 14, 18);
    ...
    doc.text(`Alertes HACCP : ${criticalCount}`, 14, 66);
    ...
    autoTable(doc, {
      startY: 80,
      head: [["Date", "Équipement", "Température", "Zone HACCP", "Statut"]],
      body: tableData,
    ...
    doc.save(`HACCP_Temperatures_${new Date().toISOString().slice(0, 10)}.pdf`);
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={() => setShowQuickAdd(false)}>
          <div className="bg-[#030b1d] border border-white/10 rounded-[28px] p-6 w-full max-w-sm mx-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <input
              ...
              value={quickAddTemp}
              onChange={(e) => setQuickAddTemp(e.target.value)}
              autoFocus
            ...
            <button
              ...
              disabled={isAdding}
              className="w-full h-12 rounded-2xl bg-cyan-400 hover:bg-cyan-300 disabled:opacity-50 transition text-black font-black text-sm flex items-center justify-center gap-2"
            >
        ...
        <div className="mx-auto max-w-[1450px] rounded-[32px] border border-white/5 bg-[#030b1d] p-5 shadow-[0_0_80px_rgba(0,150,255,0.08)] space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-4 gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 space-y-0.5 max-h-[60px] overflow-y-auto">
                    {missingLogsEquipments.length === 0 ? (
                      ...
                      missingLogsEquipments.map((eq) => (
                        <div key={eq.id} className="text-orange-200 text-xs">• {eq.name}</div>
                      ))
                    )}
                  </div>
                </div>
            ...
            <div className="rounded-[24px] border border-green-500/20 bg-gradient-to-br from-green-500/15 to-green-900/10 p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-green-300 text-sm font-semibold">Statut HACCP</p>
                  <h2 className="text-[40px] font-black text-white leading-none mt-3">
                    {alertsCount === 0 ? "ONLINE" : "ALERTE"}
                  </h2>
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/[0.02] p-5 md:p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-white/30 text-xs">Saisie manuelle HACCP</p>
              </div>
            </div>
              ...
              return (
                <div className="mt-3 p-3 rounded-2xl bg-white/[0.03] border border-white/[0.05] flex items-center gap-3">
                  ...
                  <p className="text-white/50 text-xs">
                    Zone HACCP de <span className="text-cyan-400 font-bold">{eq.name}</span> : <span className="text-white font-bold">{eq.temp_min}°C → {eq.temp_max}°C</span>
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/[0.02] p-5 md:p-6">
            ...
            {logs.length > logsLimit && (
              <div className="mt-5 text-center">
                <button
                  onClick={() => setLogsLimit((l) => l + 20)}
                  className="px-5 py-2.5 rounded-2xl border border-white/10 bg-white/[0.03] text-white/50 hover:text-white hover:border-white/20 transition text-sm font-bold flex items-center gap-2 mx-auto"
                >
          ...
          <div className="rounded-[28px] border border-cyan-500/20 bg-gradient-to-br from-cyan-500/[0.07] to-blue-900/5 p-5 md:p-6 space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-black text-white">Export HACCP PDF</h2>
                ...
                {equipments.map((equipment) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (isSelected) {
                          setSelectedEquipments(selectedEquipments.filter((eq) => eq !== equipment.name));
                        } else {
                          setSelectedEquipments([...selectedEquipments, equipment.name]);
                        }
                      }}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
              <div>
                ...
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)}
                  className="w-full h-12 rounded-2xl bg-white/[0.05] border border-white/10 px-4 text-white outline-none text-sm" />
              </div>
              <div>
