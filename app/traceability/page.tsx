...
export default function TraceabilityPage() {
  ...
  const filteredProducts = products.filter((item) => {
    const matchSearch = !search || (item.product || "").toLowerCase().includes(search.toLowerCase()) || (item.supplier || "").toLowerCase().includes(search.toLowerCase());
    const status = getDLCStatus(item.dlc);
  ...
  function exportPDF() {
    ...
    doc.text(`Périmés : ${expiredCount}`, 14, 64);
    doc.text(`Bientôt périmés : ${soonCount}`, 14, 70);

    autoTable(doc, {
      startY: 78,
      head: [["Produit", "Lot", "DLC", "Fournisseur", "Qté", "Statut"]],
        ...
        <div className="rounded-[28px] border border-white/10 bg-white/[0.02] p-5">
          <div className="flex flex-col xl:flex-row gap-3 xl:items-end xl:justify-between">
            ...
            <button
              ...
              className="h-11 px-6 rounded-2xl bg-cyan-400 hover:bg-cyan-300 disabled:opacity-50 transition text-black font-black text-sm flex items-center justify-center gap-2 shrink-0"
            >
          ...
          <div className="flex items-center justify-between gap-4 mb-5 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/30 text-xs">{filteredProducts.length} / {products.length} produits</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-2 h-10 rounded-xl bg-white/[0.05] border border-white/10 px-3 min-w-[160px]">
                ...
                <input
                  ...
                  className="bg-transparent outline-none w-full text-xs text-white placeholder:text-white/25"
                />
                  ...
                  className={`px-3 py-2 rounded-xl border text-xs font-bold transition ${
                    filterStatus === status
                      ? status === "expired" ? "bg-red-500/25 border-red-500/50 text-red-300"
                        : status === "soon" ? "bg-orange-500/25 border-orange-500/50 text-orange-300"
