...
// ─────────────────────────────────────────────
// TYPES — adaptés à ta vraie table Supabase
// ─────────────────────────────────────────────
...
function calcHours(arrival: string, departure: string): number {
  ...
  // Nettoie le format Safari qui peut envoyer "09:30 AM" ou "9:30"
...
function HoursPanel({
  ...
  function getWeeksInMonth(year: number, month: number) {
    ...
    while (current <= lastDay) {
      ...
      weeks.push({
        label: `${formatDateShort(clampStart)} — ${formatDateShort(clampEnd)}`,
        start: toISO(clampStart),
        end: toISO(clampEnd),
      });
  ...
  function exportPDF() {
    ...
    autoTable(doc, {
      ...
      body: weekLogs.map((log) => [
        ...
        formatHours(log.total_hours || 0), log.note || "—",
      ]),
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 overflow-y-auto" onClick={onClose}>
      <div className="w-full max-w-3xl rounded-[32px] border border-white/10 bg-[#030b1d] p-6 shadow-2xl my-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between gap-4 mb-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black text-white">{employee.full_name}</h2>
              <p className="text-white/40 text-xs">{employee.role} {isAdmin && <span className="text-orange-400">· Vue Admin</span>}</p>
            </div>
          </div>
          <button onClick={onClose} className="w-9 h-9 rounded-xl bg-white/5 hover:bg-white/10 flex items-center justify-center transition shrink-0"><X size={16} /></button>
        </div>
        ...
        <div className="flex gap-2 mb-4">
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition border ${activeView === tab.id ? "bg-cyan-400 text-black border-cyan-300" : "bg-white/[0.04] border-white/10 text-white/50 hover:text-white"}`}
            >
              {tab.label}
            </button>
          ))}
          ...
          <div className="space-y-3 mb-4">
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/50 text-sm font-bold px-2">{weekLabel}</span>
            ...
            {weekLogs.length > 0 && (
              <div className="grid grid-cols-7 gap-1.5">
                {weekDates.map((d, i) => {
                  ...
                  return (
                    <div key={i} className={`rounded-xl p-2 text-center border ${isToday ? "border-cyan-500/30 bg-cyan-500/10" : "border-white/[0.06] bg-white/[0.02]"}`}>
                      <p className={`text-[10px] font-bold uppercase ${isToday ? "text-cyan-400" : "text-white/30"}`}>{DAYS_FR[i].slice(0, 3)}</p>
                      <p className={`text-[10px] ${isToday ? "text-cyan-300" : "text-white/25"}`}>{d.getDate()}</p>
                      <p className={`text-sm font-black mt-1 ${dayLog ? "text-violet-300" : "text-white/15"}`}>
        ...
        {activeView === "month" && (
          <div className="space-y-3 mb-4">
            ...
            {/* Month progress bar (base 151h = ~35h x 4.33 semaines) */}
            <div className="h-2 rounded-full bg-white/[0.06] overflow-hidden">
        ...
        <div className="rounded-[20px] border border-white/10 bg-white/[0.03] p-4 mb-4 space-y-3">
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
            ...
            {/* Arrivée — 2 selects H + M */}
            <div>
              <label className="text-white/30 text-xs mb-1 block">Arrivée *</label>
              <div className="flex gap-1">
                <select
                  value={arrival.split(":")[0] || ""}
                  onChange={(e) => {
                    const h = e.target.value;
                    const m = arrival.split(":")[1] || "00";
                    setArrival(h ? `${h}:${m}` : "");
                  }}
                  className="flex-1 h-11 rounded-2xl bg-white/[0.05] border border-white/10 px-2 text-white text-sm outline-none"
                >
                ...
                <select
                  value={arrival.split(":")[1] || ""}
                  onChange={(e) => {
                    const h = arrival.split(":")[0] || "08";
                    setArrival(`${h}:${e.target.value}`);
                  }}
                  className="flex-1 h-11 rounded-2xl bg-white/[0.05] border border-white/10 px-2 text-white text-sm outline-none"
                >
            ...
            {/* Départ — 2 selects H + M */}
            <div>
              <label className="text-white/30 text-xs mb-1 block">Départ *</label>
              <div className="flex gap-1">
                <select
                  value={departure.split(":")[0] || ""}
                  onChange={(e) => {
                    const h = e.target.value;
                    const m = departure.split(":")[1] || "00";
                    setDeparture(h ? `${h}:${m}` : "");
                  }}
                  className="flex-1 h-11 rounded-2xl bg-white/[0.05] border border-white/10 px-2 text-white text-sm outline-none"
                >
                ...
                <select
                  ...
                  onChange={(e) => {
                    const h = departure.split(":")[0] || "17";
                    setDeparture(`${h}:${e.target.value}`);
                  }}
                  className="flex-1 h-11 rounded-2xl bg-white/[0.05] border border-white/10 px-2 text-white text-sm outline-none"
                >
                  <option value="">MM</option>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={saveHours}
              disabled={isSaving || !arrival.includes(":") || !departure.includes(":")}
              className="h-11 px-6 rounded-2xl bg-cyan-400 hover:bg-cyan-300 disabled:opacity-50 transition text-black font-black text-sm flex items-center gap-2"
            >
  ...
  async function handleSave(form: Partial<Employee>) {
    ...
    if (form.id) {
      await supabase.from("employees").update({
        ...
        pin_code: form.pin_code, is_active: form.is_active,
      ...
      await supabase.from("employees").insert({
        full_name: form.full_name, role: form.role,
        pin_code: form.pin_code, is_active: form.is_active ?? true,
        email: form.email || null,
      });
        ...
        <div className="mx-auto max-w-[1450px] rounded-[32px] border border-white/5 bg-[#030b1d] p-5 shadow-[0_0_80px_rgba(0,150,255,0.08)] space-y-5">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2 mt-2">
              {isAdmin ? (
                <>
                  <button onClick={() => { setEditingEmployee(null); setShowEmployeeModal(true); }} className="h-10 px-4 rounded-2xl bg-cyan-400 hover:bg-cyan-300 transition text-black text-sm font-black flex items-center gap-2">
                    <UserPlus size={15} /> Ajouter
                  </button>
                  <button onClick={() => { setIsAdmin(false); addToast("Mode admin désactivé", "success"); }} className="h-10 px-4 rounded-2xl border border-orange-500/30 bg-orange-500/10 text-orange-300 hover:bg-orange-500/20 transition flex items-center gap-2 text-sm font-bold">
                    <LogOut size={14} /> Admin
                  </button>
                </>
              ) : (
                <button onClick={() => setShowAdminPin(true)} className="h-10 px-4 rounded-2xl border border-orange-500/20 bg-orange-500/[0.07] hover:bg-orange-500/15 text-orange-300 transition flex items-center gap-2 text-sm font-bold">
            ...
            <div className="rounded-[24px] border border-green-500/20 bg-gradient-to-br from-green-500/15 to-green-900/10 p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="text-[48px] font-black text-white leading-none mt-3">{activeEmployees.length}</h2>
          ...
          <div className="flex gap-2 border-b border-white/[0.06]">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-5 py-3 rounded-t-2xl text-sm font-bold transition border-b-2 -mb-px ${activeTab === tab.id ? "border-cyan-400 text-cyan-400 bg-cyan-500/[0.07]" : "border-transparent text-white/40 hover:text-white/70"}`}>
              ...
              {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {employees.map((employee) => (
                    <div key={employee.id} className={`rounded-[24px] border p-5 transition-all ${employee.is_active ? "border-green-500/20 bg-gradient-to-br from-green-500/[0.06] to-green-900/5" : "border-white/[0.08] bg-white/[0.02]"}`}>
                      ...
                      <div className="space-y-1.5 mb-4">
                        ...
                        {employee.email && (
                          <div className="flex items-center gap-2">
                            <span className="text-white/40 text-xs truncate">{employee.email}</span>
                          </div>
                        )}
                      </div>
