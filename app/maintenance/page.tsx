...
import {
  Wrench, AlertTriangle, CheckCircle2, Clock3, Plus, X, Loader2,
  FileDown, Camera, Upload, Trash2, Eye, RefreshCw, ShieldCheck,
  Calendar, ChevronDown, ChevronUp, Bell,
} from "lucide-react";
...
interface MaintenanceReport {
  id: number; equipment_name: string; description: string;
  priority: Priority; status: Status; reported_by: string;
  assigned_to?: string; created_at: string; updated_at: string;
}

interface Certificate {
  id: number; name: string; equipment: string; file_url: string;
  maintenance_date: string; next_date: string; created_at: string;
}
...
function ReportCard({ report, isAdmin, onStatusChange, onDelete }: {
  report: MaintenanceReport; isAdmin: boolean;
      ...
      <div className="flex items-center justify-between gap-3 p-3 sm:p-4">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-2xl flex items-center justify-center shrink-0 border ${
            ...
            report.priority === "Urgent" ? "bg-orange-500/20 border-orange-500/30" : "bg-blue-500/15 border-blue-500/20"
          }`}>
            <Wrench size={15} className={report.priority === "Critique" ? "text-red-300" : report.priority === "Urgent" ? "text-orange-300" : "text-blue-300"} />
          </div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${getPriorityColor(report.priority)}`}>
                {report.priority === "Critique" && <AlertTriangle size={8} className="mr-1" />}{report.priority}
              </span>
            </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className={`hidden sm:inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold border ${getStatusColor(report.status)}`}>{report.status}</span>
      ...
      {expanded && (
        <div className="px-3 sm:px-4 pb-4 border-t border-white/[0.05] pt-3 space-y-3">
          <div className="flex sm:hidden">
            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold border ${getStatusColor(report.status)}`}>{report.status}</span>
          </div>
          ...
          {isAdmin && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {(["En attente", "En cours", "Résolu"] as Status[]).map((s) => (
                <button key={s} onClick={() => onStatusChange(report.id, s)} className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition ${report.status === s ? getStatusColor(s) : "border-white/10 bg-white/[0.03] text-white/40 hover:text-white/70"}`}>{s}</button>
              ))}
              <button onClick={() => onDelete(report.id)} className="ml-auto h-8 w-8 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-400 flex items-center justify-center transition"><Trash2 size={13} /></button>
            </div>
          )}
        </div>
      )}
...
export default function MaintenancePage() {
  ...
  async function handleRefresh() {
    setIsRefreshing(true); await fetchData(); setIsRefreshing(false);
    addToast("Données actualisées", "success");
  }

  function checkAdminPin() {
    if (pinInput === adminPin) {
      setIsAdmin(true); setShowAdminInput(false); setPinInput(""); setPinError(false);
  ...
  async function saveCertificate() {
    ...
    if (certFile) {
      ...
      const { error: uploadError } = await supabase.storage.from("certificates").upload(fileName, certFile, { contentType: certFile.type });
      setUploadProgress(80);
      if (uploadError) { addToast(`Erreur upload : ${uploadError.message}`, "error"); setIsSavingCert(false); setUploadProgress(0); return; }
        ...
        <div className="mx-auto max-w-[1450px] rounded-[24px] sm:rounded-[32px] border border-white/5 bg-[#030b1d] p-4 sm:p-5 shadow-[0_0_80px_rgba(0,150,255,0.08)] space-y-4 sm:space-y-5">
            ...
            <div className="flex items-center gap-2 mt-1 shrink-0">
              ...
              {isAdmin ? (
                ...
                <div className="flex items-center gap-2">
                  {showAdminInput ? (
                    <>
                      <input type="password" maxLength={4} value={pinInput} onChange={(e) => setPinInput(e.target.value.replace(/\D/g, ""))} onKeyDown={(e) => e.key === "Enter" && checkAdminPin()} placeholder="PIN" className={`h-10 w-16 sm:w-20 rounded-2xl border px-3 text-white text-sm outline-none text-center ${pinError ? "border-red-500/60 bg-red-500/10" : "border-white/10 bg-white/[0.05]"}`} />
                      ...
                      <button onClick={() => { setShowAdminInput(false); setPinInput(""); }} className="h-10 w-10 rounded-2xl border border-white/10 bg-white/[0.03] text-white/50 flex items-center justify-center transition"><X size={14} /></button>
                    </>
                  ) : (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
            <div className={`rounded-[24px] border p-4 sm:p-5 ${pendingCount > 0 ? "border-orange-500/30 bg-gradient-to-br from-orange-500/15 to-orange-900/10" : "border-orange-500/20 bg-gradient-to-br from-orange-500/10 to-orange-900/5"}`}>
              <div className="flex items-start justify-between gap-3">
                <div><p className="text-orange-300 text-xs font-semibold">En attente</p><h2 className="text-[36px] sm:text-[42px] font-black text-white leading-none mt-2">{pendingCount}</h2><p className="text-orange-400/50 text-xs mt-1">pannes déclarées</p></div>
                <div className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-2xl border border-orange-400/20 bg-orange-500/20 shrink-0"><Clock3 size={16} className={`text-orange-300 ${pendingCount > 0 ? "animate-pulse" : ""}`} /></div>
              </div>
            </div>
            <div className={`rounded-[24px] border p-4 sm:p-5 ${criticalCount > 0 ? "border-red-500/40 bg-gradient-to-br from-red-500/20 to-red-900/10" : "border-red-500/20 bg-gradient-to-br from-red-500/10 to-red-900/5"}`}>
              <div className="flex items-start justify-between gap-3">
                <div><p className="text-red-300 text-xs font-semibold">Critiques</p><h2 className="text-[36px] sm:text-[42px] font-black text-white leading-none mt-2">{criticalCount}</h2><p className="text-red-400/50 text-xs mt-1">non résolues</p></div>
                <div className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-2xl border border-red-400/20 bg-red-500/20 shrink-0"><AlertTriangle size={16} className={`text-red-300 ${criticalCount > 0 ? "animate-pulse" : ""}`} /></div>
              </div>
            </div>
            <div className="rounded-[24px] border border-green-500/20 bg-gradient-to-br from-green-500/15 to-green-900/10 p-4 sm:p-5">
              <div className="flex items-start justify-between gap-3">
                <div><p className="text-green-300 text-xs font-semibold">Résolues</p><h2 className="text-[36px] sm:text-[42px] font-black text-white leading-none mt-2">{resolvedCount}</h2><p className="text-green-400/50 text-xs mt-1">au total</p></div>
                <div className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-2xl border border-green-400/20 bg-green-500/20 shrink-0"><CheckCircle2 size={16} className="text-green-300" /></div>
              </div>
            </div>
          ...
          <div className="flex gap-2 border-b border-white/[0.06] overflow-x-auto">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 sm:px-5 py-3 rounded-t-2xl text-sm font-bold transition border-b-2 -mb-px whitespace-nowrap ${activeTab === tab.id ? "border-orange-400 text-orange-400 bg-orange-500/[0.07]" : "border-transparent text-white/40 hover:text-white/70"}`}>
                ...
                {tab.id === "pannes" && pendingCount > 0 && <span className="ml-1 px-1.5 py-0.5 rounded-full bg-red-500 text-white text-[10px] font-black">{pendingCount}</span>}
              </button>
            ))}
          </div>
          ...
          {activeTab === "pannes" && (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div className="flex gap-2 flex-wrap w-full sm:w-auto">
                  ...
                  {(["all", "Critique", "Urgent", "Normal"] as const).map((p) => (
                    <button key={p} onClick={() => setFilterPriority(p)} className={`px-3 py-2 rounded-xl border text-xs font-bold transition whitespace-nowrap ${filterPriority === p ? p === "Critique" ? "bg-red-500/20 border-red-500/40 text-red-300" : p === "Urgent" ? "bg-orange-500/20 border-orange-500/40 text-orange-300" : p === "Normal" ? "bg-blue-500/20 border-blue-500/40 text-blue-300" : "bg-white/10 border-white/20 text-white" : "border-white/10 text-white/35 hover:text-white/60"}`}>
              ...
              {showReportForm && (
                <div className="rounded-[24px] border border-orange-500/20 bg-orange-500/[0.05] p-4 sm:p-5 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                    <div><label className="text-white/30 text-xs mb-1 block">Équipement *</label><input value={equipmentName} onChange={(e) => setEquipmentName(e.target.value)} placeholder="Ex: Friteuse, Hotte, Frigo..." className="w-full h-11 rounded-2xl bg-white/[0.05] border border-white/10 px-4 text-white text-sm outline-none" /></div>
                    <div><label className="text-white/30 text-xs mb-1 block">Déclaré par *</label><input value={reportedBy} onChange={(e) => setReportedBy(e.target.value)} placeholder="Votre nom" className="w-full h-11 rounded-2xl bg-white/[0.05] border border-white/10 px-4 text-white text-sm outline-none" /></div>
                    <div><label className="text-white/30 text-xs mb-1 block">Priorité *</label>
                  ...
                  <div><label className="text-white/30 text-xs mb-1 block">Description *</label><textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Décrivez la panne en détail..." rows={3} className="w-full rounded-2xl bg-white/[0.05] border border-white/10 px-4 py-3 text-white text-sm outline-none resize-none" /></div>
                  <button onClick={saveReport} disabled={isSavingReport} className="h-11 px-6 rounded-2xl bg-orange-400 hover:bg-orange-300 disabled:opacity-50 transition text-black font-black text-sm flex items-center gap-2">
                    {isSavingReport ? <Loader2 size={15} className="animate-spin" /> : <Bell size={15} />} Envoyer l'alerte
                  </button>
                </div>
              )}
              ...
              {isLoading ? (
                ...
                <div className="rounded-[28px] border border-white/10 bg-white/[0.02] p-12 text-center"><Wrench size={36} className="mx-auto mb-3 text-white/20" /><p className="text-white/30 text-sm">Aucune panne déclarée</p></div>
              ) : (
                <div className="space-y-3">
                  {filteredReports.map((report) => (
                    <ReportCard key={report.id} report={report} isAdmin={isAdmin} onStatusChange={updateStatus} onDelete={deleteReport} />
                  ))}
                </div>
              )}
            </div>
          ...
          {activeTab === "certificats" && (
            <div className="space-y-4">
              ...
              {showCertForm && (
                <div className="rounded-[24px] border border-cyan-500/20 bg-cyan-500/[0.05] p-4 sm:p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div><label className="text-white/30 text-xs mb-1 block">Nom du certificat *</label><input value={certName} onChange={(e) => setCertName(e.target.value)} placeholder="Ex: Entretien hotte 2024" className="w-full h-11 rounded-2xl bg-white/[0.05] border border-white/10 px-4 text-white text-sm outline-none" /></div>
                    <div><label className="text-white/30 text-xs mb-1 block">Équipement *</label><input value={certEquipment} onChange={(e) => setCertEquipment(e.target.value)} placeholder="Ex: Hotte, Frigo 1..." className="w-full h-11 rounded-2xl bg-white/[0.05] border border-white/10 px-4 text-white text-sm outline-none" /></div>
                    <div><label className="text-white/30 text-xs mb-1 block">Date d'entretien</label><input type="date" value={certMaintenanceDate} onChange={(e) => setCertMaintenanceDate(e.target.value)} className="w-full h-11 rounded-2xl bg-white/[0.05] border border-white/10 px-4 text-white text-sm outline-none" /></div>
                    <div><label className="text-white/30 text-xs mb-1 block">Prochain entretien</label><input type="date" value={certNextDate} onChange={(e) => setCertNextDate(e.target.value)} className="w-full h-11 rounded-2xl bg-white/[0.05] border border-white/10 px-4 text-white text-sm outline-none" /></div>
                  </div>
                  <div>
                    <label className="text-white/30 text-xs mb-2 block">Document (optionnel)</label>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <label className="cursor-pointer">
                        <input type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) { setCertFile(f); addToast(`Photo sélectionnée : ${f.name}`, "success"); }}} />
                        <div className="h-14 rounded-2xl border border-cyan-500/30 bg-cyan-500/10 hover:bg-cyan-500/20 transition flex items-center justify-center gap-3 text-cyan-300 font-bold text-sm"><Camera size={18} /> Prendre une photo</div>
                      </label>
                      <label className="cursor-pointer">
                        <input type="file" accept="image/*,application/pdf" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) { setCertFile(f); addToast(`Fichier sélectionné : ${f.name}`, "success"); }}} />
                        <div className="h-14 rounded-2xl border border-white/10 bg-white/[0.04] hover:bg-white/[0.08] transition flex items-center justify-center gap-3 text-white/60 hover:text-white font-bold text-sm"><Upload size={18} /> Choisir un fichier</div>
                      </label>
                    </div>
                    {certFile && (
                  ...
                  <button onClick={saveCertificate} disabled={isSavingCert} className="h-11 px-6 rounded-2xl bg-cyan-400 hover:bg-cyan-300 disabled:opacity-50 transition text-black font-black text-sm flex items-center gap-2">
                    {isSavingCert ? <Loader2 size={15} className="animate-spin" /> : <CheckCircle2 size={15} />} Enregistrer le certificat
                  </button>
                </div>
              )}
              ...
              {isLoading ? (
                ...
                <div className="rounded-[28px] border border-white/10 bg-white/[0.02] p-12 text-center"><ShieldCheck size={36} className="mx-auto mb-3 text-white/20" /><p className="text-white/30 text-sm">Aucun certificat enregistré</p></div>
              ) : (
                    ...
                    return (
                      <div key={cert.id} className={`rounded-[24px] border p-4 sm:p-5 transition-all ${expired ? "border-red-500/30 bg-gradient-to-br from-red-500/10 to-red-900/5" : soon ? "border-orange-500/25 bg-gradient-to-br from-orange-500/[0.08] to-orange-900/5" : "border-green-500/20 bg-gradient-to-br from-green-500/[0.06] to-green-900/5"}`}>
                        <div className="flex items-start justify-between gap-3 mb-3">
                          ...
                          <div className="flex-1 min-w-0"><h3 className="text-white font-black text-sm break-words">{cert.name}</h3><p className="text-white/40 text-xs">{cert.equipment}</p></div>
                        ...
                        <div className="space-y-1.5 mb-4">
                          {cert.maintenance_date && <div className="flex items-center gap-2 text-xs text-white/40"><Calendar size={11} className="shrink-0" />Entretien : <span className="text-white/60">{formatDateFR(cert.maintenance_date)}</span></div>}
                          {cert.next_date && <div className="flex items-center gap-2 text-xs text-white/40"><Clock3 size={11} className="shrink-0" />Prochain : <span className={`font-bold ${expired ? "text-red-400" : soon ? "text-orange-400" : "text-green-400"}`}>{formatDateFR(cert.next_date)}</span></div>}
                        </div>
                        <div className="flex items-center gap-2">
                          {cert.file_url && <a href={cert.file_url} target="_blank" rel="noopener noreferrer" className="flex-1 h-9 rounded-xl border border-cyan-500/20 bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 transition text-xs font-bold flex items-center justify-center gap-1.5"><Eye size={12} /> Voir le doc</a>}
                          {cert.file_url && <a href={cert.file_url} download className="h-9 w-9 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] text-white/60 hover:text-white flex items-center justify-center transition"><FileDown size={13} /></a>}
                          {isAdmin && <button onClick={() => deleteCertificate(cert.id)} className="h-9 w-9 rounded-xl border border-red-500/20 bg-red-500/10 hover:bg-red-500/20 text-red-400 flex items-center justify-center transition"><Trash2 size={13} /></button>}
                        </div>
                      </div>
                    );
                  })}
